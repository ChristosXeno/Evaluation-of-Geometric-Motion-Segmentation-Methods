...
if nargout>2
  ...
  % Process B if it contains inf or nan
  B_processed = B;
  B_processed(isinf(B_processed)|isnan(B_processed)) = 0;
  
  % Process resnrm if it contains inf or nan
  resnrm_processed = resnrm;
  resnrm_processed(isinf(resnrm_processed)|isnan(resnrm_processed)) = 0;
  
  % Compute singular vectors
  [P,S,Q] = svd(full([B_processed;[zeros(1,j-1),resnrm_processed]]),0); 
  S = diag(S);
  if size(Q,2)~=k
    Q = Q(:,1:k); 
    P = P(:,1:k); 
